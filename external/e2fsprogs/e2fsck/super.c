...
#define API_LEVEL_ROS 30
	if (sb->s_snapshot_r_blocks_count != API_LEVEL_ROS) {
		log_out(ctx, _("s_snapshot_r_blocks_count is %llu."
			" It might be upgraded from Q OS. Reinitialize it to 30\n"),
			(unsigned long long)sb->s_snapshot_r_blocks_count);
		sb->s_snapshot_r_blocks_count = API_LEVEL_ROS;
